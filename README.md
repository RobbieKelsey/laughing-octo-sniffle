...
- **Flows:** authenticate, discover, issue & redeem E-pass, **cross-module pass**, E-pass prebooking, register membership & redeem, membership prebooking, eSIM purchase & top-up, fitness booking & check-in, sandbox redemption simulation, **cancel & refund a booking**
- **Screens:** the booking flow now has a dedicated **Review & pay** step (steps indicator, collapsible cancellation policy, itemised bill) before confirming; **Order history** (status/category filter chips) drills into an **order detail** screen with cancel/refund actions; **Membership → Entitlements** shows per-module usage bars; **Profile → Settings** and a webhook-driven **Notifications** feed (bell icon, unread badge) round out the account area
- **Redemption codes:** clicking any order (from Wallet or Order history) opens its redemption code — the actual identifier a venue scans, sourced straight from the order (`voucher` for prebookings/eSIM/fitness, `dpId` for walk-ins and sandbox redemptions) — with a QR / Barcode / Image switcher so you can see how the same code renders in each format
- **Atom-inspired UI:** the app preview borrows patterns from DragonPass's atom-ui-mobile component library — a Steps indicator, Tag pills, Collapse, a Bill/order-summary block, and a bottom-sheet Dialog for destructive confirmations — layered onto the existing Midnight Blue / Coral Red brand

### Schema fidelity

Every call in the console carries a badge:

- **✓ schema** — request/response field names verified against the developer portal. These are exact: `POST /v2/auth/token`, `POST /v2/users`, `GET /v2/search/modules`, `GET /v2/resources` (list), `GET /v2/resources/{resourceId}` (details), `POST /v2/resources/prebookings/availability`, `POST /v2/orders/{module}/ePasses`, `POST /v2/orders/multiModule/ePasses`, `POST /v2/orders/{module}/ePasses/prebooking` (incl. Fitness & eSIM), `POST /v2/orders/{module}/memberships/prebooking`, `POST /v2/orders/{module}/memberships/prebooking/preview`, `POST /v2/orders/ePasses/details` & `/v2/orders/memberships/details` (usage), `POST /v2/orders/ePasses` (order list, now with optional `status`/`category` filters), `POST /v2/orders/fitness/checkin`, `POST /v2/orders/esims/ePasses/topup`, `GET /v2/resources/esims/{resourceId}/options`, `GET /v2/resources/fitness/{resourceId}/schedule`, `POST /v2/entitlements/update`, `POST /v2/memberships`, `POST /v2/memberships/qrCodes`, `POST /v2/users/ePasses` (user e-pass list), `POST /v2/simulations/lounges/redemptions`, plus both webhook payloads — `walkin.redemption` and `prebooking.statusChanged` (also reused, with `currentStatus:3`, for the order-cancelled event).
- **≈ shape** — the membership-side eSIM/fitness order variants remain modelled on their verified E-pass equivalents; the new order-management endpoints `POST /v2/orders/{module}/cancel` and `POST /v2/orders/{module}/refund` are representative only — no equivalent was pulled from the portal, so treat their exact field names as illustrative. Everything else the demo exercises by default is verified.
